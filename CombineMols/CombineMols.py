...
RDLogger.DisableLog('rdApp.*') #for skip RDKit messeages
...
def CombineMols(mol1, mol2, dummy=53):
    ...
    CombineMols( (Mol/str)mol1, (Mol/str)mol2, (int,str)dummy) -> list:
        Returns list of possible combinations of combined molecules
            Arguments:
                ...
                - dummy: (int of atomic num)/str of atomic symbol) The atomic number or symbol of the dummy atom, which indicates the atom you want to bond with.
            Returns:
                Mol
    """
    ...
    if str(type(mol1)) == "<class 'str'>":
        mol1 = Chem.MolFromSmiles(mol1)
    if str(type(mol2)) == "<class 'str'>":
    ...
    for atom in mol1.GetAtoms():
        if atom.GetAtomicNum() == dummy:
            ...
            bonds.append( (atom.GetIdx(), bond.GetBeginAtomIdx() if atom.GetIdx()!=bond.GetBeginAtomIdx() else bond.GetEndAtomIdx()) )
